...
class Tool{
 ...
 protected:
  ...
  void MLC() {*(m_data->Log)<<MsgL(0,m_verbose);}  ///< Function for clearing logging level
  template <typename T>  void Log(T message, int messagelevel, int verbosity){m_data->Log->Log(message,messagelevel,verbosity);}
  template <typename T>  void Log(T message, int messagelevel=0){m_data->Log->Log(message,messagelevel,m_verbose);}  ///< Logging fuction for printouts. @param message Templated message string. @param messagelevel The verbosity level at which to show the message. Checked against internal verbosity level
